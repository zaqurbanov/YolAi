...
// Per-mission ENERGY reward for the daily quests (0097_daily_quest_split.sql):
// each of today's 3 rotating missions pays this amount, claimed individually
// via claimDailyMissionAction. Single-field control mirroring
// GlobalDailyCoinGrantControl — load on mount, PATCH { dailyMissionReward },
// reset-to-default deletes the app_settings row.
export default function DailyMissionRewardControl() {
  const [settings, setSettings] = useState<DailyMissionRewardSettings | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    async function load() {
      const res = await fetch('/api/admin/chat-meta?type=daily-mission-reward');
      if (res.ok && !cancelled) {
        const data: DailyMissionRewardSettings = await res.json();
        setSettings(data);
        setInputValue(String(data.dailyMissionReward));
      }
  ...
  async function save(dailyMissionReward: number | null) {
    setPending(true);
    setError(null);
    try {
      const res = await fetch('/api/admin/chat-meta?type=daily-mission-reward', {
        ...
        body: JSON.stringify({ dailyMissionReward }),
      });
      ...
      {!loading && (
        <div className="flex items-center gap-2 flex-wrap">
          <TextField
            ...
            className="w-32"
            aria-label="Gündəlik missiya mükafatı"
          >
