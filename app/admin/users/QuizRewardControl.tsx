...
interface QuizRewardSettings {
  dailyQuizReward: number;
  source: 'table' | 'default';
}

export default function QuizRewardControl() {
  const [settings, setSettings] = useState<QuizRewardSettings | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    async function load() {
      const res = await fetch('/api/admin/chat-meta?type=daily-quiz-reward');
      if (res.ok && !cancelled) {
        const data: QuizRewardSettings = await res.json();
        setSettings(data);
        setInputValue(String(data.dailyQuizReward));
      }
  ...
  async function save(dailyQuizReward: number | null) {
    setPending(true);
    setError(null);
    try {
      const res = await fetch('/api/admin/chat-meta?type=daily-quiz-reward', {
        ...
        body: JSON.stringify({ dailyQuizReward }),
      });
  ...
  return (
    <div className="rounded-3xl border border-border/40 bg-surface p-6 shadow-sm flex flex-col sm:flex-row sm:items-center gap-4 sm:justify-between">
      <div>
        <div className="text-[12px] font-bold uppercase tracking-[0.1em] text-navy">Bugünkü sual mükafatı (enerji)</div>
        {loading ? (
          <Skeleton className="h-6 w-32 mt-2 rounded-full" />
        ) : (
          <div className="mt-1 flex items-center gap-2">
            <span className="editorial-display text-xl font-bold leading-none tabular-nums text-primary">{settings?.dailyQuizReward}</span>
            <span
      ...
      {!loading && (
        <div className="flex items-center gap-2 flex-wrap">
          <TextField
            ...
            className="w-32"
            aria-label="Bugünkü sual mükafatı"
          >
