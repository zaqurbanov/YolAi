...
const HUD_FRAMES: { key: keyof Omit<CoinQazanPage3DProps, 'coinBalance' | 'streakDays'>; label: string; span?: string }[] = [
  ...
  { key: 'gamesSection', label: 'OYUNLAR TERMİNALI' },
  { key: 'wheelGame', label: 'ÇARX' },
];

function HudFrame({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="hud-glass relative flex flex-col gap-3 overflow-hidden rounded-2xl border-l-4 p-1 sm:p-1.5" style={{ borderColor: 'var(--hud-primary)' }}>
      <div
      ...
      <div className="px-1 pb-1 sm:px-1.5 sm:pb-1.5">{children}</div>
    </div>
  );
}
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-5xl flex-col gap-6">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          {HUD_FRAMES.map(({ key, label }) => (
            <HudFrame key={key} label={label}>
              {cards[key]}
            </HudFrame>
          ))}
        </div>
